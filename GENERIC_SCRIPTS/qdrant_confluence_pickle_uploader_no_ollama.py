...
def load_config():
    ...
    # Required settings check (removed ollama-related requirements)
    required_settings = [
    ...
    settings['vector_size'] = NOMIC_VECTOR_SIZE  # Fixed for Nomic model
    ...
    # Override embed_model to indicate we're using direct embedding
    settings['embed_model'] = 'nomic-embed-text-v1.5-direct'
...
def process_confluence_page(page_data: Dict, space_key: str, space_name: str,
                          config: Dict, pg_conn, qdrant_client, embed_model,
    ...
    page_metadata = {
        ...
        'embed_model': 'nomic-embed-text-v1.5-direct'
    }
    ...
    try:
        # Encode all chunks at once (much faster than one by one)
        with torch.no_grad():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload Confluence pickle files to OpenWebUI WITHOUT Ollama (using sentence-transformers)"
    )
    ...
    parser.add_argument("--vector-size", type=int, default=NOMIC_VECTOR_SIZE)
    ...
    parser.add_argument("--base-url", default=config.get('base_url', 'https://confluence.example.com'),
    ...
    # Load checkpoint (SAME FILE as the Ollama version for continuity)
    checkpoint = load_checkpoint(config['checkpoint_file'])
    uploaded_files = checkpoint.get('uploaded_files', [])
    processed_spaces = checkpoint.get('processed_spaces', {})
    
    print("="*60)
    print("Confluence Pickle Uploader (NO OLLAMA - Direct Embeddings)")
    print("="*60)
    print(f"Using device: {args.device}")
    print(f"Checkpoint file: {config['checkpoint_file']}")
    print(f"Continuing from previous checkpoint with {len(uploaded_files)} files already processed")
    ...
    print(f"\nLoading Nomic embedding model with sentence-transformers...")
    print(f"Model: {NOMIC_MODEL_NAME}")
    print(f"Device: {args.device}")
    
    try:
        embed_model = SentenceTransformer(NOMIC_MODEL_NAME, device=args.device)
        print(f"Model loaded successfully! Vector size: {NOMIC_VECTOR_SIZE}")
        
        # Test the model
        test_embedding = embed_model.encode("test", convert_to_numpy=True)
    ...
    ensure_collection_exists(qdrant_client, FILES_COLLECTION, NOMIC_VECTOR_SIZE)
    ensure_collection_exists(qdrant_client, KNOWLEDGE_COLLECTION, NOMIC_VECTOR_SIZE)
        ...
        try:
            ...
            for idx, page in enumerate(pages):
                ...
                success, file_info = process_confluence_page(
                    ...
                    qdrant_client, embed_model, args.base_url, all_pages_lookup
                )
    ...
    # Don't clear checkpoint - let user decide
    print(f"\nCheckpoint preserved at: {config['checkpoint_file']}")
    print("To start fresh next time, use --clear-checkpoint")
