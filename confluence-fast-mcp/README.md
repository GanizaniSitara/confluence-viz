...
python ../convert_to_json.py              # one-time: convert pickles to JSON
./simple-server --data-dir ../json_data
```

Same 5 MCP tools as the Python version, compiled to a single binary.
...
python ../convert_to_json.py              # one-time: convert pickles to JSON
./server --data-dir ../json_data          # builds index on first run
```

Or build the index separately:
```bash
./server build-index --data-dir ../json_data --index-dir ./index_data
./server --data-dir ../json_data --index-dir ./index_data
```
...
| `convert_to_json.py` | Converts pickles to JSON (for Go servers) |
...
Both Go servers read JSON files from `json_data/` (generated by `convert_to_json.py`).
