...
- **Python server**: Python 3.8+, FastMCP 3.0+
- **Go server**: Go 1.21+ (no Python needed at runtime)
...
pip install fastmcp beautifulsoup4 lxml pydantic requests python-dateutil whoosh3
```
...
Reads `.pkl` files directly from `../temp/` and defaults to HTTP on port 8070 — same as the Python version.
...
### Option 1: Python Server (WHOOSH Search)
...
### Option 2: Go Server (TF-IDF index, no WHOOSH)
...
- Same tool output format as Python server
...
```bash
python server.py                   # streamable HTTP on port 8070 (default)
python server.py --sse             # legacy SSE transport (if needed)
...
Or via the CLI:
```bash
claude mcp add --transport http --scope user confluence-fast http://YOUR_IP:8070/mcp
```
...
Reads `.pkl` pickle files directly (via gopickle) — no conversion step needed.
Same 5 MCP tools, same output format — drop-in replacement for the Python version.
