...
def find_optimal_clusters(spaces, max_clusters=20):
    """
    Determine the optimal number of clusters using multiple methods:
    1. Elbow Method
    2. Silhouette Analysis
    
    Returns a recommended number of clusters, and displays a plot.
    """
    print("\nAnalyzing optimal number of clusters...")
    X, valid_spaces = get_vectors(spaces)
    X_array = X.toarray()
    
    # Minimum clusters is 2, maximum is either specified max or 1/3 of data points
    max_possible = min(max_clusters, len(valid_spaces) // 3)
    if max_possible < 2:
        max_possible = min(max_clusters, len(valid_spaces))
    
    range_n_clusters = range(2, max_possible + 1)
    ...
    for n_clusters in range_n_clusters:
        ...
        # (Silhouette score requires at least 2 clusters and more than n_clusters samples)
        if n_clusters > 1 and n_clusters < len(X_array):
    ...
    # For elbow method - find the point of maximum curvature
    # Calculate the second derivative (acceleration) to find the "elbow"
    if len(inertias) > 2:
        deltas = np.diff(inertias, 2)  # Second derivative
        elbow_idx = np.argmin(deltas) + 2  # +2 due to taking diff twice
        elbow_n = range_n_clusters[elbow_idx]
    else:
        elbow_n = 2  # Default if not enough data points
    
    # For silhouette method - find the maximum score
    if silhouette_scores:
        sil_idx = np.argmax(silhouette_scores)
        sil_n = range_n_clusters[sil_idx]
    else:
        sil_n = 2  # Default if not enough data points
    
    # If methods disagree, pick an average
    if elbow_n == sil_n:
        optimal_n = elbow_n
    else:
        # Give more weight to silhouette score
        optimal_n = int(round(0.7 * sil_n + 0.3 * elbow_n))
    
    # Make the result 1-indexed instead of 0-indexed for display
    print(f"\nRecommended number of clusters:")
    print(f"  Elbow Method: {elbow_n}")
    print(f"  Silhouette Method: {sil_n}")
    print(f"  Final recommendation: {optimal_n}")
...
def cluster_spaces(spaces, method='agglomerative', n_clusters=5):
    ...
    # Make labels 1-indexed instead of 0-indexed
    labels = labels + 1
...
def main():
    ...
    ┌─────────────────────────────────────────────────────────┐
    │                                                         │
    │   CONFLUENCE CLUSTER EXPLORER v2                        │
    │   =============================                         │
    │                                                         │
    │   Analyze and visualize Confluence spaces               │
    │   Version 2.0 - With Optimal Clustering                 │
    │                                                         │
    └─────────────────────────────────────────────────────────┘
    """)
    print("="*80 + "\n")
    print("Welcome to the Confluence Cluster Explorer v2!")
    while True:
        ...
        print(f"5. Find optimal number of clusters (Elbow & Silhouette methods)")
        ...
        if choice == '1':
            ...
            try:
                if not spaces:
                    ...
                    continue
                optimal_clusters = find_optimal_clusters(spaces)
                n_clusters = optimal_clusters  # Update the current setting
                print(f"Number of clusters has been set to the optimal value: {n_clusters}")
            except Exception as e:
                print(f"An error occurred while finding optimal clusters: {e}")
        elif choice == '6':
            try:
